              ...
              <v-layout row>
                ...
                <v-flex xs4 sm4 style="height:60px">
                  <div
                    ...
                    style="display: flex;align-items: center;justify-content: center;"
                  >
                    <v-icon style="margin-top:20px;font-size:30px">fas fa-spinner</v-icon>
                  </div>
                  <v-card
...
export default class IndividualMatchDetails extends Vue {
  ...
  async withdrawBonus() {
    try {
      ...
      let result = await withdrawBonusTx.request([
        {
          comment:
  ...
  async updateYourBet() {
    ...
    if (this.match.status == MatchStatus.finished) {
      if (
        ...
        let output = await getBetMethod.caller(acc.address).call(this.matchId);
  ...
  async withdrawBet(seedId: MatchSeedIds, seedName: string) {
    try {
      ...
      let result = await withdrawBetTx.request([
        {
